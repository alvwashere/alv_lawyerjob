...
CreateThread( function()
    if Config.CheckVersion then
  	...
  	PerformHttpRequest('https://raw.githubusercontent.com/alvwashere/ScriptVersions/master/lawyerjob_version', function(code, res, headers)
  		if code == 200 then
  			local rv = json.decode(res)
  			if tonumber(table.concat(mysplit(rv.version, "."))) > tonumber(table.concat(mysplit(version, "."))) then
  					print(([[
^6[ALV-GG]^0 Lawyer Script Script Loaded
